#!/usr/bin/env python3
"""라인·공·허들 최종 판단과 fusion 상태를 한 화면에 요약하는 ROS2 노드."""
...
class VisionStatusMonitor(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            "VisionStatusMonitor started: line_result, ball_result, hurdle_result"
        )
    ...
    def print_summary(self) -> None:
        ...
        if self.hurdle_state is not None and self._fresh("hurdle_state"):
            hurdle_extra = (
                ...
                f" rs={int(bool(self.hurdle_state.get('realsense_valid', False)))}"
                f" cam={int(bool(self.hurdle_state.get('webcam_valid', False)))}"
                f" cross={int(bool(self.hurdle_state.get('crossing_ready', False)))}"
            )
